    ...
    color("firebrick")
      translate([0, 0, BASE_HEIGHT])
        linear_extrude(fill_height_real)
          rounded_square(
            foreach_add(grid_size_mm, -d_wall / 2),
            BASE_TOP_RADIUS,
            center=true
          );
    children();
...
module gridfinityBase(grid_size, grid_dimensions = GRID_DIMENSIONS_MM, hole_options = bundle_hole_options(), off = 0, final_cut = true, only_corners = false, thumbscrew = false) {
  ...
  assert(
    is_bool(final_cut) && is_bool(only_corners) && is_bool(thumbscrew)
  );
...
module base_solid(top_dimensions = BASE_TOP_DIMENSIONS) {
  ...
  assert(
    sweep_inner.x > 0 && sweep_inner.y > 0,
...
module profile_cutter(h, l, s) {
  ...
  translate([r_f2, r_f2])
    hull() {
      ...
      difference() {
        translate([l - scoop - 2 * r_f2, scoop]) if (scoop != 0) {
          intersection() {
            circle(scoop);
